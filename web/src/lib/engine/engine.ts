...
export function runEngine({ ast, index, outGraph, assumptions, ctx, fnRegistry, objectSchema, overrides }) {
  ...
  for (const alias of outGraph.order) {
    try {
      ...
      for (let i = 0; i < args.length; i++) {
        ...
        if (arg.kind === 'ref') {
          ...
          if (index.aliases.has(arg.name)) {
            ...
            if (!series) {
              throw new Error(`Engine: missing input "${key}" for ${objName}.${alias}`);
            }
            inputs.push(series);
            ...
            if (srcAliases.length === 0) {
              throw new Error(`Engine: reference to object "${arg.name}" but it has no outputs`);
            }
            ...
            for (const srcAlias of srcAliases) {
              ...
              const series = store.get(key);
              if (!series) {
                throw new Error(`Engine: missing input "${key}" (expanded from object "${arg.name}")`);
              }
            ...
            throw new Error(`Engine: unknown reference "${arg.name}.${arg.field}"`);
          }
      ...
      const outputAss = (objAss.outputs && objAss.outputs[alias]) || {};

      // we MUST have channels in schema
      const schema = objectSchema[objNode.fnName];
      if (!schema || !schema.channels) {
        throw new Error(`Engine: unknown object type "${objNode.fnName}" or missing channels in schema.`);
      }
      const declaredChannels = Object.keys(schema.channels);
      if (declaredChannels.length === 0) {
        throw new Error(`Engine: object type "${objNode.fnName}" declares zero channels.`);
      }
      ...
      if (returnedADeclaredChannel) {
        for (const [name, series] of resultEntries) {
          if (!declaredChannels.includes(name)) continue;
        ...
        if (!aliasSeries) {
          throw new Error(`Engine: function for "${objName}" did not return expected data.`);
        }
        if (declaredChannels.length === 1) {
          ...
          throw new Error(`Engine: function for "${objName}" returned unnamed series but multiple channels declared.`);
        }
      }
      ...
      console.warn(`Engine error for alias "${alias}":`, e.message);

      const objName = outGraph.aliasOwner(alias);
      const depsForThisAlias = outGraph.dependents.get(alias) || new Set();

      if (depsForThisAlias.size > 0) {
        // Critical: something depends on this broken output
        throw new Error(`Critical Error in "${alias}" (${objName}): ${e.message}. This output is used by: ${Array.from(depsForThisAlias).join(', ')}`);
      } else {
        // Warning: isolated failure
        warnings.push({
          type: 'engine_warning',
          message: `Object "${objName}" could not be calculated: ${e.message}. Results for this object will be missing.`,
          object: objName,
          alias: alias
        });
      }
  ...
  return { store, byAlias, order: outGraph.order, warnings };
}